package ru.projects.alexchekmenev.vkplaylister;

import com.vk.sdk.api.VKApiConst;

/**
 * Created by creed on 14.10.15.
 */
public final class Constants extends VKApiConst {
    public static final String VK_ACCESS_TOKEN = "VK_ACCESS_TOKEN";

    // VK API requests
    public static final String VK_AUDIO_GET = "audio.get";
    public static final String VK_AUDIO_SEARCH = "audio.search";
    public static final String VK_AUDIO_GET_BY_ID = "audio.getById";
    public static final String VK_AUDIOS_ADD_ALBUM = "audio.addAlbum";
    public static final String VK_AUDIOS_GET_ALBUMS = "audio.getAlbums";
    public static final String VK_AUDIOS_MOVE_TO_ALBUM = "audio.moveToAlbum";
    public static final String VK_AUDIOS_DELETE_ALBUM = "audio.deleteAlbum";

    public static final String VK_ALBUM_ID = "album_id";
    public static final String VK_ALBUM_TITLE = "title";
    public static final String VK_AUDIO_IDS = "audio_ids";
    public static final String VK_AUDIOS = "audios";
    public static final String VK_AUTO_COMPLETE = "auto_complete";
    public static final String VK_LYRICS = "lyrics";
    public static final String VK_PERFORMER_ONLY = "performer_only";
    public static final String VK_SEARCH_OWN = "search_own";

}
